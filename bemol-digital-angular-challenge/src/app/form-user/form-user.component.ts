...
import { UserService } from '../pages/user-pages/user.service';
...
export class FormUserComponent implements OnInit {
  ...
  constructor(
    ...
    private cepapiServive: CepApiService
  ) {}
  ...
  save() {
    let userDTO = new UserDTOModel(
      ...
      this.form.value.cpf,
      this.form.value.phone.substring(0, 2),
      this.form.value.phone.substring(2, 11),
      this.form.value.birth,
      this.form.value.postcode,
      this.form.value.st,
      this.form.value.city,
      this.form.value.neighborhood,
  ...
  new(userDTO: UserDTOModel) {
    this.userService.store(userDTO).subscribe(
      (response) => {
        ...
        this.ngxNotifierService.createToast(
          'Usuário atualizado',
          'success',
          5000
        );
